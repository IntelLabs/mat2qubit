...
class qSymbOp_test(unittest.TestCase):
    ...
    def test_str_read_1(s):
        ...
        s.assertEqual(
            symop1.getCoeff(opstringTuple), 1.0
        )  # Otherwise should be (numeric,'string')
        ...
        with s.assertRaises(IndexError):
            symop1.getCoeff(
                (("1", "Y"),)
            )  # Since operator 'Y' isn't one of allowed ops
    ...
    def test_str_read_3(s):
        symop2 = qSymbOp.qSymbOp(
            "[X_1 Z_v2] ++ 2*k [Y_1 Z_v3] ++ - y [Z_1 Z_v3 bigop_v4]"
        )
        opstringTuple1 = (
            ("1", "X"),
            ("v2", "Z"),
    ...
    def test_subs(s):
        ...
        symop2.scalar_subs({"k": 2.0, "y": 7.0})
