...
ketbra_dict = dict(
    [
        ("k{}b{}".format(a, b), "k{}b{}".format(a, b))
        for a, b in itertools.product(range(dmax_deflt), range(dmax_deflt))
    ]
)
...
def symbop_pauli_to_mat(inpString, ssname_order):
    ...
    pauli_defs = {"X": qopmats.qub["X"], "Y": qopmats.qub["Y"], "Z": qopmats.qub["Z"]}

    dVals = dict(zip(ssname_order, [2] * len(ssname_order)))
    encodings = dict(zip(ssname_order, ["stdbinary"] * len(ssname_order)))

    compOp = symbop_to_dlevcompositeop(
        symbop, ssname_order, dVals, encodings, pauli_defs
    )
...
def symbop_to_QubitOperator(inpSymbop, ssname_order, dvals, encodings, inpOpChars={}):
    ...
    dlevecompos = symbop_to_dlevcompositeop(
        inpSymbop, ssname_order, dvals, encodings, inpOpChars
    )
