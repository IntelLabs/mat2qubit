...
def integerstring_to_bitstring(intstring, sysdlev):
    ...
    assert isinstance(
        sysdlev, compositeDLevels
    )  # compositeOperator is a subclass of compositeDLevels
    ...
    # Remember, lmax = d-1
    bstring = [
        i2b.int2bits(intstring[i], dvals[i] - 1, encs[i], encparams[i])
...
def parseBraket(f):
    ...
    while line:
        ...
        ketArr = (
            np.array([ssid_val.split(":") for ssid_val in ket.split(",")]).astype(int).T
        )
        braArr = (
            np.array([ssid_val.split(":") for ssid_val in bra.split(",")]).astype(int).T
        )

        # assert ketArr.shape==braArr.shape  # Same num terms in ket and bra
        assert (
            ketArr[0, :].tolist() == braArr[0, :].tolist()
        )  # Same subsystem ids in ket and bra
...
def parseBraketToQasmBuilder(f):
    ...
    while line:
        ...
        if line.strip() == "BREAK":
            ...
            # Ket and Bra, 2xL arrays with {ssid,level}
            ketArr = (
                np.array([ssid_val.split(":") for ssid_val in ket.split(",")])
                .astype(int)
                .T
            )
            braArr = (
                np.array([ssid_val.split(":") for ssid_val in bra.split(",")])
                .astype(int)
                .T
            )
