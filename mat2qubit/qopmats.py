...
qub["I2"] = np.eye(2)
qub["Z"] = np.array([[1.0 + 0j, 0], [0, -1]])
qub["X"] = np.array([[0j, 1.0], [1.0, 0]])
qub["Y"] = np.array([[0, -1.0j], [1.0j, 0]])
qub["sig+"] = np.array([[0.0 + 0j, 1], [0, 0]])
qub["sig-"] = np.array([[0.0 + 0j, 0], [1, 0]])
qub["n"] = np.array([[0.0 + 0j, 0], [0, 1]])
qub["H"] = 1.0 / np.sqrt(2) * np.array([[1 + 0j, 1], [1, -1]])
qub["S"] = np.array([[1.0, 0], [0, 1j]])
qub["T"] = np.array([[1.0, 0], [0, np.exp(1j * np.pi / 4)]])
...
def momQho(n):
    a = an(n)
    return 1 / (1.0j * np.sqrt(2.0)) * (a - a.T)


def posQho(n):
    a = an(n)
    return 1 / np.sqrt(2.0) * (a + a.T)
...
def spinY(s, hbar=1.0):
    ...
    for a in range(1, N):
        ...
        Sy[a - 1, b - 1] = -1.0j * np.sqrt((s + 1) * (a + b - 1) - a * b)
