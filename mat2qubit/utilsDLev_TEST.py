...
class dlevelutils_tests(unittest.TestCase):
    ...
    def test_int_and_bin_strings(s):
        ...
        s.assertRaises( IndexError, utilsDLev.integerstring_to_bitstring, ints_outofbounds,compos_op )
    ...
    def test_intstrings_to_state(s):
        ...
        # Unary
        psi_notnrml = np.array([0,1,1,0,1,0,0,0.]) # 001,010,100
        ...
        # SB
        psi_notnrml = np.array([1,1,1,0.]) # 00,01,10 (reverse indexing)
        psi = psi_notnrml/np.sqrt(3)
        enc = 'stdbinary'
        res_nn = utilsDLev.int_strings_to_psi_notnormalized( int_strings, d,enc )
        res    = utilsDLev.int_strings_to_psi( int_strings, d,enc )
        ...
        # Gray 
        psi_notnrml = np.array([1,1,0,1.]) # 00,01,11 (reverse indexing)
        psi = psi_notnrml/np.sqrt(3)
        enc = 'gray'
        res_nn = utilsDLev.int_strings_to_psi_notnormalized( int_strings, d,enc )
        res    = utilsDLev.int_strings_to_psi( int_strings, d,enc )
    ...
    def test_permutations(s):
        ...
        sites = 2
        d = 2
        gold_state_nn = np.array([0,1,1,0.])
