...
dmax_builtinops = 100
builtInOps =    ["ident","numop","Nsq","qhoCr","qhoAn",
                "qhoPos","qhoMom", "q","p",
                "Qsq","Psq",  "p2",
                "q2","q3","q4","q5","q6", # Powers of position operator
                "a","ad",'n', # Creation/annihilation ops
                "Sx","Sy","Sz",
                "X_1stQ"] \
            +   [ "Pr{}".format(a) for a in range(dmax_builtinops) ] \
            +   [ "k{}b{}".format(a,b) for a,b in itertools.product(range(dmax_builtinops),range(dmax_builtinops)) ]
    ...
    def opToMatrixRep(s,inpOp):
        ...
        if isinstance(inpOp,np.ndarray):
            ...
            if  inpOp=="ident":
                ...
                return qopmats.spinX( float(s.d-1)/2. )
            elif inpOp=="Sy":
                return qopmats.spinY( float(s.d-1)/2. )
            elif inpOp=="Sz":
                return qopmats.spinZ( float(s.d-1)/2. )
            elif inpOp=="X_1stQ":
            ...
            elif len(re.findall(r"k\d+b\d+$",inpOp))==1:  # KetBra ('^' and $' are 'anchors' in regex)
        ...
        for i in range(s.d):
            for j in range(s.d):
                ...
                for bitId,boolUseBit in enumerate(mask):
                    if boolUseBit==False: continue

                    # Possible inputs: |0><0|,|0><1|,|1><0|,|1><1|
                    pauliString *= sglQubOp( I[bitId], J[bitId], 
                                             bitId+s.qubitShift )
...
class compositeDLevels():
    ...
    def opStringToMatRep(s,coeff,opString, ignore_encoding=False):
        ...
        if ignore_encoding:
            ...
            mats_by_ssid = dict( [(i,spr.eye(ssize)) for (i,ssize) in zip(range(num_ss),[ss.d for ss in s.subsystems])] )
            if opString!="ident":
            ...
            # [::-1] for reverse order
            spr_matrep_opstr = coeff*functools.reduce( spr.kron, list(mats_by_ssid.values())[::-1] )
...
class compositeOperator(compositeDLevels):
    ...
    def __init__(s,inpCompositeSys=None,defaultEnc=None):
        ...
        assert( inpCompositeSys==None or 
            isinstance(inpCompositeSys,compositeDLevels) )
