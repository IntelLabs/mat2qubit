...
class i2b_tests(unittest.TestCase):
    ...
    def test_spin(s):
        ...
        x1_2 = 1.0 / 2 * np.array([[0, 1], [1, 0]], dtype=complex)
        y1_2 = 1.0 / 2 * np.array([[0, -1.0j], [1.0j, 0]], dtype=complex)
        z1_2 = 1.0 / 2 * np.array([[1, 0], [0, -1]], dtype=complex)
        ...
        x5_2[0, 1] = np.sqrt(5.0)
        x5_2[1, 2] = 2 * np.sqrt(2)
        x5_2[2, 3] = 3.0
        x5_2[3, 4] = 2 * np.sqrt(2)
        x5_2[4, 5] = np.sqrt(5.0)
    ...
    def test_qub(s):
        ...
        s.assertEqual(
            qopmats.qub["Z"].tolist(), np.array([[1.0 + 0j, 0], [0, -1]]).tolist()
        )
        s.assertEqual(
            qopmats.qub["X"].tolist(), np.array([[0j, 1.0], [1.0, 0]]).tolist()
        )
        s.assertEqual(
            qopmats.qub["Y"].tolist(), np.array([[0, -1.0j], [1.0j, 0]]).tolist()
        )
        s.assertEqual(
            qopmats.qub["sig+"].tolist(), np.array([[0.0 + 0j, 1], [0, 0]]).tolist()
        )
        s.assertEqual(
            qopmats.qub["sig-"].tolist(), np.array([[0.0 + 0j, 0], [1, 0]]).tolist()
        )
        s.assertEqual(
            qopmats.qub["n"].tolist(), np.array([[0.0 + 0j, 0], [0, 1]]).tolist()
        )
